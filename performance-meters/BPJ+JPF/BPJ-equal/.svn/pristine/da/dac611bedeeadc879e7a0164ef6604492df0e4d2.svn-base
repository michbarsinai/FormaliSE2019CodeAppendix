...
public class TraceGenerator {
	...
	void logToVisualizationTrace() {
		...
		writeXmlFile(bp.getName()+".xml",doc);
	}
	...
	void logHierarchyTable (Element eventData) {
		...
		for (BThread bt : bp.allBThreads.values()) {
	...
	void logDependencyTable (Element eventData) {
		...
		for (BThread sc : bp.allBThreads.values()) {
			if (sc.requestedEvents != null) {
				
				Iterator<RequestableInterface> iter = sc.requestedEvents.iterator();
				...
				for (Event e :  sc.requestedEvents.getEventList()) {
					...
					for (BThread watcher : bp.allBThreads.values()) {
					...
					for (BThread blocker : bp.allBThreads.values()) {
